...
// DOWNSTREAM PATCH: libc++ before version 19 advertises __cpp_lib_ranges >=
// 202202L but does not actually provide std::ranges::range_adaptor_closure
// (which was added to libc++ in version 19), so excluding such libc++ versions
// here makes the selection below fall through to the library's own
// implementation. Without this the build fails on, e.g., macOS with clang 15 /
// libc++ 15. See https://github.com/llvm/llvm-project/issues/70557.
#if defined(__cpp_lib_ranges) && 202202L <= __cpp_lib_ranges &&               \
    (!defined(_LIBCPP_VERSION) || 190000 <= _LIBCPP_VERSION)
